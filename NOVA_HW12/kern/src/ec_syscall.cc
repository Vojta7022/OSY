...
void Ec::syscall_handler(uint8 a)
{
    ...
    {
        // ABI:
        // esi = start routine entry point
        // edi = user stack top

        void (*start_routine)(void) =
            reinterpret_cast<void (*)(void)>(r->esi);
        ...
        if (!ec)
        {
            r->eax = 1; // allocation failure
            break;
        }

        Sys_regs *nr = ec->sys_regs();
        ...
        nr->edx = reinterpret_cast<mword>(start_routine);
        nr->ecx = stack_top;
    ...
    case sys_thr_yield:
    {
        ...
        // Switch to next execution context (never returns)
        cur->next->make_current();
        UNREACHED;
    }
